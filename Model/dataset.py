...
class TrafficLightDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        light_mode = torch.tensor(int(self.labels.iloc[index, 1]), dtype=torch.long) #mode of the traffic light
        block = self.labels.iloc[index, 6] #label of blocked or unblocked
        points = self.labels.iloc[index, 2:6] #midline coordinates
        points = [points[0]*(480/4032), points[1]*(480/3024), points[2]*(480/4032), points[3]*(480/3024)]
        points = [p / 480 for p in points] #normalize coordinate values to be between [0,1]
...
#                cp[0] = (657-shifty-b)/m
        ...
        image = transforms.functional.normalize(image, mean = [120.56737612047593/255, 119.16664454573734/255, 113.84554638827127/255], std=[66.32028460114392/255, 65.09469952002551/255, 65.67726614496246/255])
