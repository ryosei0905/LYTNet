...
# テスト用の逆正規化関数（normalize した画像を表示用に戻す）
def unnormalize(img_tensor):
    mean = torch.tensor([120.56737612047593/255, 119.16664454573734/255, 113.84554638827127/255]).view(3, 1, 1)
    std = torch.tensor([66.32028460114392/255, 65.09469952002551/255, 65.67726614496246/255]).view(3, 1, 1)
...
def test():
    ...
    test_file_loc = './Annotations/testing_file.csv'
    test_image_directory = '../PTL/{0}/test'.format(IMAGE_SIZE)
    MODEL_PATH = '../results/{0}/{1}/{2}/{3}/{4}/{4}_epoch_{5}'.format(LYTNET, IMAGE_SIZE, NORMALIZE, PARAMETER, BATCH_SIZE, EPOCH)
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(dataloader):
            ...
            if (predicted == mode).sum().item() == 0:
                ...
                '''
                # GPU→CPU, 正規化解除, numpy変換
                image_tensor = unnormalize(images[0].cpu()).clamp(0, 1)  # ← 0〜1に収める
                image = (image_tensor.permute(1, 2, 0).numpy() * 255).astype(np.uint8)  # PIL用に戻す
                ...
                display_image(image, title, pred_points, gt_points, 120) #factor is 120 because 4*120 = 480
