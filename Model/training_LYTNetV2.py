...
train_img_dir = '../PTL/{0}/train'.format(IMAGE_SIZE)
valid_img_dir = '../PTL/{0}/val'.format(IMAGE_SIZE)
save_path = '../results/LYTNetV2/{0}/{1}/{2}/{3}/{3}'.format(IMAGE_SIZE, NORMALIZE, PARAMETER, BATCH_SIZE)
...
def train():
    for epoch in range(MAX_EPOCHS):
        ...
        for j, data in enumerate(tqdm(train_dataloader, desc=f"Epoch {epoch+1}/{MAX_EPOCHS}", unit="batch")):
        ...
        print('Training accuracy: ' + str(100*train_correct/train_total))
        ...
        print('Angle Error:', angle_error/train_total)
        print('Startpoint Error:', startpoint_error/train_total)
        print('Endpoint Error:', endpoint_error/train_total)
        ...
        with torch.no_grad():
            ...
            for k, data in enumerate(tqdm(valid_dataloader, desc=f"Validation {epoch+1}/{MAX_EPOCHS}", unit="batch")):
            ...
            if epoch == 50:
                ...
                torch.save(states, save_path + '_epoch_50')
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig(save_path + '_losses.png')

    #plots training and validation cross entropy loss
    plt.figure()
    plt.title('Train and Validation Cross Entropy Loss')
    plt.plot(train_CE, label='Train CE Loss')
    plt.plot(valid_CE, label='Validation CE Loss')
    plt.xlabel('Epoch')
    plt.ylabel('Cross Entropy Loss')
    plt.legend()
    plt.savefig(save_path + '_cross_entropy.png')
    ...
    plt.ylabel('MSE')
    plt.legend()
    plt.savefig(save_path + '_mse.png')
...
if __name__ == '__main__':
    ...
    multiprocessing.freeze_support()  # これは念のため
    train()
